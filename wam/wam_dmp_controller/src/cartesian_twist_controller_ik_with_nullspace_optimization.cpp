...
#include <kdl/chainfksolvervel_wdls.hpp>
...
namespace wam_dmp_controller
{
    ...
    CartesianTwistControllerWithNullspaceOptimization::CartesianTwistControllerWithNullspaceOptimization() : jnt_to_twist_solver_(NULL), jnt_to_pose_solver_(NULL), jnt_to_jac_solver_(NULL), num_joints(0), publisher_counter_(0), publisher_buffer_size_(0), head_sequence_number_(0)
    {
    }
    ...
    bool CartesianTwistControllerWithNullspaceOptimization::init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &node_handle)
    {
        ...
        if (!(pose_filter_.configure(NUM_CART, node_handle_.getNamespace() + std::string("/pose_filter"), node_handle_)))
        {
    ...
    bool CartesianTwistControllerWithNullspaceOptimization::readParamters()
    {
    ...
    bool CartesianTwistControllerWithNullspaceOptimization::initMechanismChain()
    {
        ...
        {
            ROS_ERROR("Could not extract chain between %s and %s from the kdl tree.", root.c_str(), tip.c_str());
            return false;
        }
    ...
    bool CartesianTwistControllerWithNullspaceOptimization::initNullspacePidControllers()
    {
        ...
        {
            ROS_ERROR("Could not retrieve parameter >>null_space_controller_names<< from the parameter server in the namespace %s!", node_handle_.getNamespace());
            return false;
        }
        ...
        {
            ROS_ERROR("There are %i nullspace terms but there should be %i", nullspace_fb_pid_controllers_.size(), num_joints_);
            return false;
        }
    ...
    bool CartesianTwistControllerWithNullspaceOptimization::initJointPositionController(hardware_interface::EffortJointInterface *hw, ros::NodeHandle node_handle, std::vector<JointPositionController>& joint_position_controllers)
    {
        ...
        for (int i = 0; i < (int)joint_names.size(); i++)
        {
            ...
            joint_position_controllers.push_back(joint_position_controller);
        }

        if (ROS_ASSERT(static_cast<int>(joint_position_controllers.size()) == NUM_JOINTS))
        {
            return true;
        }
        else 
        {
            ROS_ERROR("There should be >>%i<< controllers, instead there are >>%i<< controllers!", NUM_JOINTS, joint_position_controllers.size());
            return false;
        }
    }
    ...
    void CartesianTwistControllerWithNullspaceOptimization::starting(const ros::Time& time)
    {
        ...
        for (unsigned int i = 0; i < joint_position_controllers_.size(); i++)
        {
            ...
            {
                ROS_DEBUG("Starting: The type for joint >>%i<< is Continuous!");
    ...
    void CartesianTwistControllerWithNullspaceOptimization::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        for (unsigned int i = 0; i < joint_position_controllers_.size(); i++)
        {
            ...
            if (joint_position_controllers_[i].getJointType() == urdf::Joint::CONTINUOUS)
            {
                ROS_DEBUG("Updating: The type for joint >>%i<< is Continuous!");
                kdl_current_joint_positions_(i) = angles::normalize_angle(kdl_current_joint_positions_(i));
                eigen_desired_joint_positions_(i) = angles::normalize_angle(eigen_desired_joint_positions_);
            }
        ...
        eigen_nullspace_projector_ = eigen_identity_ - (eigen_jac_pseudo_inverse_ * eigen_chain_jac_);
        ...
        for (int i = 0; i < NUM_CART; i++)
        {
            pose_unfiltered_data_[i] = kdl_twist_error_;
        }
        ...
        eigen_nullspace_joint_velocities_ += eigen_nullspace_term_;
        ...
        for (int i = 0; i < num_joints_; i++)
        {
            joint_position_controllers_[i].update(); // Joint limits are enforced here 
        }
        ...
        for (int i = 0; i < num_joints_; i++)
        {
            kdl_current_joint_velocities_(i) = joint_position_controllers_[i].getJointVelocity();
        }
