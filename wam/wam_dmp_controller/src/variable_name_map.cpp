/*************************************************************************
	...
	> Created Time: Thu 08 Jun 2017 04:57:27 PM PDT
 ************************************************************************/
...
namespace wam_dmp_controller
{
                    ...
                    const int start_index)
    {
        ...
        for (int i = start_index_; i < start_index_ + supported_variable_names.size(); i++)
        {
    ...
    bool VariableNameMap::set(const std::string& used_variable_names, const int index)
    {
        assert(initialized_);
        ...
        std::tr1::unordered_map<std::string, int>::iterator it = supported_name_to_index_map_.find(used_variable_names);
        if (it == supported_name_to_index_map_.end())
        {
            ROS_ERROR("Could not find the used variable name!");
            return false;
        }

        int supported_index;
        if(!getSupportedVariableIndex(used_variable_names, supported_index))
        {
            ROS_ERROR("Could not match >%s<!", used_variable_names[i].c_str());
            return false;
        }
    ...
    {
        assert(initialized_);
        if ((supported_index >= 0) && (used_index < (int)supported_to_used_map_.size()))
        {
            if (supported_to_used_map_[supported_index] >= 0)
            {
                used_index = supported_to_used_map_[supported_index];
                return true;
            }
        }
        else 
        {
            ROS_ERROR("Invalid supported_index!");
            return false;
        }
    }
    ...
    {
        std::tr1::unordered_map<std::string, int>::iterator it = supported_name_to_index_map_.find(name);
        if (it !=supported_name_to_index_map_.end())
        {
            index = it->second;
            return true;
